...
export default{
  ...
  methods: {
    async fetchStudents() {
      ...
      const order = 'asc';
      try {
        const response = await fetch(`https://api.proste20.ru/students?sort_by=${sortBy}&order=${order}`);
    ...
    async fetchLectures() {
      try {
        const response = await fetch('https://api.proste20.ru/lectures');
        const data = await response.json();
    ...
    async addStudent() {
      ...
      if (this.newStudent.trim()) {
        try {
          const response = await fetch('https://api.proste20.ru/students', {
            method: 'POST',
            headers: {
    ...
    async removeLecture(lectureId) {
      try {
        const response = await fetch(`https://api.proste20.ru/lectures/${lectureId}`, {
          method: 'DELETE',
        });
    ...
    async removeStudent(studentId) {
      try {
        const response = await fetch(`https://api.proste20.ru/students/${studentId}`, {
          method: 'DELETE',
        });
    ...
    //};
      ...
      try {
        console.log(this.attendance)
          const response = await fetch('https://api.proste20.ru/attendance', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
