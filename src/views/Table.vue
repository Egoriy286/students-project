...
export default {
  ...
  methods: {
    ...
    async fetchAttendanceData() {
      const response = await fetch('http://api.proste20.ru/attendance?sort_by=${sortBy}&order=${order}'); // Замените на ваш API
      if (!response.ok) {
    ...
    async fetchLectures() {
      const response = await fetch('http://api.proste20.ru/lectures'); // Замените на ваш API для получения лекций
      if (!response.ok) {
    ...
    async removeAttendance(AttendanceId) {
      try {
        ...
        const response = await fetch(`http://api.proste20.ru/attendance/${AttendanceId}`, {
          method: 'DELETE',
          mode: 'cors',
        });
